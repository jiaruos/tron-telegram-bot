...
class GetNowBlock extends Command
{
    ...
    public function blockNumber($blockByNumber=0){
        $opts = [
            "http" => [
                "method" => "GET",
                "header" => "TRON-PRO-API-KEY:674a8821-b97d-45e6-9bc9-1904061490a0"
            ]
        ];
        $context = stream_context_create($opts);
        if($blockByNumber > 0){
            $result = file_get_contents("https://api.trongrid.io/wallet/getblockbynum?num=".$blockByNumber, false, $context);
        }else{$result = file_get_contents("https://api.trongrid.io/wallet/getnowblock", false, $context);
        }
